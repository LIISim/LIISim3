...
void DataTableWidget::updateView()
{
    ...
    else
    {
        ...
        for(int i = 0; i < curves->size(); i++)
        {
            ...
            columnNames << QString("%0 (X%1)").arg(curves->at(i)->title().text()).arg(i);

            for(int j = 0; j < curves->at(i)->data()->size(); j++)
                model->setItem(j, column_counter, new QStandardItem(QString::number(curves->at(i)->data()->sample(j).x(), 'g', _number_precision)));

            column_counter++;

            columnNames << QString("%0 (Y%1)").arg(curves->at(i)->title().text()).arg(i);
...
void DataTableWidget::setXAxisNonTimeType(bool nonTime)
{
    actionInterpolate->setEnabled(!nonTime);
